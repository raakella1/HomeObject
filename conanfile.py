...
class HomeObjectConan(ConanFile):
    ...
    def package(self):
        ...
        copy(
            self,
            "*.h*",
            join(self.source_folder, "src", "include"),
            join(self.package_folder, "include"),
            keep_path=True,
        )

    def package_info(self):
        self.cpp_info.components["homestore"].requires = [
            "homestore::homestore",
            "iomgr::iomgr",
            "sisl::sisl",
        ]
        ...
        if self.options.sanitize:
            self.cpp_info.components["memory"].sharedlinkflags.append(
                "-fsanitize=address"
            )
            ...
            self.cpp_info.components["memory"].exelinkflags.append(
                "-fsanitize=undefined"
            )
